...
{
//printf("pinlatch: %d %d \n",page_no , bt->mgr->latchmgr->latchhash);
...
SYSTEM_INFO sysinfo[1];
	...
	mgr->idx = open ((char*)name, O_RDWR | O_CREAT | O_DIRECT | O_SYNC, 0666);
	...
	latchmgr = _mm_malloc (BT_maxpage,BT_maxpage);
	*amt = 0;
	...
	printf("mgr->page_size:%d, mgr->page_bits:%d, mgr->seg_bits:%d, mgr->mode:%d, mgr->poolcnt:%d, mgr->poolmax:%d, mgr->poolmask:%d, mgr->hash_size:%d\n",mgr->page_size, mgr->page_bits, mgr->seg_bits, mgr->mode, mgr->poolcnt, mgr->poolmax, mgr->poolmask, mgr->hashsize);
...
void syncmmap(BtDb* bt, BtPool* pool){
	msync (pool->map, (bt->mgr->poolmask+1) << bt->mgr->page_bits, MS_SYNC);	
}
...
BtPageSet set[1];
uid new_page;
BtPage aligned_page=(BtPage)_mm_malloc(BT_maxpage,BT_maxpage);
int reuse;
...
#ifdef unix
	memcpy(aligned_page,page,bt->mgr->page_size);
	if( pwrite(bt->mgr->idx, aligned_page, bt->mgr->page_size, new_page << bt->mgr->page_bits) < bt->mgr->page_size ){
		printf("pwrite error\n");
	...
	memset(aligned_page,0,BT_maxpage);
	if( !reuse && bt->mgr->poolmask > 0 && (new_page & bt->mgr->poolmask) == 0 )
	{
		// use zero buffer to write zeros
		if( pwrite(bt->mgr->idx,aligned_page, bt->mgr->page_size, (new_page | bt->mgr->poolmask) << bt->mgr->page_bits) < bt->mgr->page_size ){
			printf("pwrite zero failed\n");
			return bt->err = BTERR_wrt, 0;
		}
	}
#else
...
uid left;
	...
	if( !(left = bt_newpage(bt, root->page)) ){
		printf("newpage error in splitroot\n");
		return bt->err;
	}
...
void* bt_insertkey_proxy(void* arg){
	char* key=(char*)malloc(256*sizeof(char));
	InsertThreadInfo* args=arg;
	int ops_per_thread=args->ops_per_thread;
	for(int i=0;i<ops_per_thread;i++){
		sprintf(key, "%d", args->keys[i]);
		if(bt_insertkey(args->bt,key,strlen(key),0,args->id,*args->tod)){
			printf("failed to insert in initialization");
			exit(0);
		}
		printf("i %d\n", i);
	}
...
fprintf(stderr, "finished %d records , %d search, %d found, %d write\n", n, find_cnt, found, write_cnt);
...
#endif
ThreadArg *args;
InsertThreadInfo *insertargs;
uint poolsize = 0;
float elapsed, build_elapsed;
int num = 0;
char key[1];
BtMgr *mgr;
BtKey ptr;
BtDb *bt;
...
#endif
	args = malloc (cnt * sizeof(ThreadArg));
	insertargs = malloc (cnt * sizeof(InsertThreadInfo));
	...
	//for( idx = 0; idx < cnt; idx++ ) {
	//	insertargs[idx].bt = bt;
	//	insertargs[idx].keys=(int*)malloc(ops_per_thread*sizeof(int));
	//	memcpy(insertargs[idx].keys,insertion_ops+idx*ops_per_thread,ops_per_thread*sizeof(int));
	//	insertargs[idx].id = idx;
	//	insertargs[idx].tod = tod;
	//	insertargs[idx].ops_per_thread = ops_per_thread;
			
//#ifdef unix
//		if( err = pthread_create (threads + idx, NULL, bt_insertkey_proxy, insertargs + idx) )
//			fprintf(stderr, "Error creating thread %d\n", err);
//#else
//		threads[idx] = (HANDLE)_beginthreadex(NULL, 65536, bt_insertkey_proxy, insertargs + idx, 0, NULL);
//#endif
//	}
#ifdef unix
	//for( idx = 0; idx < cnt; idx++ )
...
#endif
	build_elapsed = getCpuTime(0) - start;
	fprintf(stderr, " insertion time: %dm%.3fs\n", (int)(build_elapsed/60), build_elapsed - (int)(build_elapsed/60)*60);
	start=getCpuTime(0);
	...
	elapsed = getCpuTime(0) - start;
	fprintf(stderr, " real %dm%.3fs\n", (int)(elapsed/60), elapsed - (int)(elapsed/60)*60);
	fprintf(stderr, " Insertion %f KOPS/s \n", ops_cnt/1000.0/build_elapsed);
	fprintf(stderr, " Mixed %f KOPS/s \n", ops_cnt/1000.0/elapsed);
	elapsed = getCpuTime(1);
	fprintf(stderr, " user %dm%.3fs\n", (int)(elapsed/60), elapsed - (int)(elapsed/60)*60);
	elapsed = getCpuTime(2);
	fprintf(stderr, " sys  %dm%.3fs\n", (int)(elapsed/60), elapsed - (int)(elapsed/60)*60);
