...
#ifndef NVM_NVME_BLK_IO_THREAD_ACCESSOR_H
#define NVM_NVME_BLK_IO_THREAD_ACCESSOR_H
 ...
 * we have request_queue and io_queue two levels of queues.
 * pending_requests means the number of requests in io_queue
 *
 * @tparam K key tupe
...
template<typename K, typename V, int CAPACITY>
class nvme_blk_io_thread_accessor : public nvme_blk_accessor<K, V, CAPACITY> {
...
public:
    nvme_blk_io_thread_accessor(const int &block_size, const int queue_length = 100) :
...
//        cache_ = nullptr;
    ...
    virtual int close() {
        nvme_blk_accessor<K, V, CAPACITY>::close();
        terminate_io_thread();
    }

    virtual int read(const blk_address &blk_addr, void *buffer) {
        printf("read function in nvme_blk_io_thread_accessor is called\n");
    ...
    virtual int write(const blk_address &blk_addr, void *buffer) {
        printf("write function in nvme_blk_io_thread_accessor is called\n");
        uint64_t start = ticks();
    ...
    virtual atomic<int>* get_pending_requests_() override {
        return &pending_requests_;
    }

    virtual void asynch_read(const blk_address& blk_addr, void* buffer, call_back_context* context) override {
        uint64_t start = ticks();
        io_request request;
        request.addr = blk_addr;
        request.buffer = buffer;
    ...
    virtual void super_asynch_read(const blk_address& blk_addr, void* buffer, call_back_context* context) {
        nvme_blk_accessor<K, V, CAPACITY>::asynch_read(blk_addr, buffer, context);
    }

    virtual void asynch_write(const blk_address& blk_addr, void* buffer, call_back_context* context) override {
        uint64_t start = ticks();
        io_request request;
        request.addr = blk_addr;
        request.buffer = buffer;
    ...
    virtual void super_asynch_write(const blk_address& blk_addr, void* buffer, call_back_context* context) {
        nvme_blk_accessor<K, V, CAPACITY>::asynch_write(blk_addr, buffer, context);
    }
    ...
    virtual int super_process_completion(int max = 0) {
        return nvme_blk_accessor<K, V, CAPACITY>::process_completion(max);
    }

    // request_queue -> io_queue
    static void working_thread(nvme_blk_io_thread_accessor *accessor) {
        ...
        while (!accessor->terminiating_flag_ || accessor->get_request_queue_size() > 0 || accessor->pending_requests_.load() > 0) {
            while (accessor->get_request_queue_size() > 0 && accessor->io_queue_length_ - accessor->pending_requests_.load() > 0) {
                ...
                if (request.type == read_request) {
                    ...
                    accessor->super_asynch_read(request.addr, request.buffer, request.context);
                } else {
                    //accessor->qpair_->submit_write_operation(request.buffer, accessor->block_size,
                    //                                         request.addr, callback_function, para);
                    accessor->super_asynch_write(request.addr, request.buffer, request.context);
                }
            }
            ...
            //    }
            //}
            int processed=accessor->super_process_completion();
            accessor->current_results_.fetch_add(processed);
        }
    ...
    struct callback_para {
        ...
        blk_address addr;
        io_request request;
        nvme_blk_io_thread_accessor* accessor;
    };
    ...
    virtual int process_completion(int max = 0) override {
        return current_results_.exchange(0);
    }
...
#endif //NVM_NVME_BLK_IO_THREAD_ACCESSOR_H
