...
#ifndef NVM_IO_THREAD_B_PLUS_TREE_H
#define NVM_IO_THREAD_B_PLUS_TREE_H
...
#include "../blk/nvme_blk_io_thread_accessor.h"
...
namespace tree {
    ...
    class io_thread_b_plus_tree : public pull_based_b_plus_tree<K, V, CAPACITY> {
    public:
        io_thread_b_plus_tree(const int block_size, int queue_length):
                pull_based_b_plus_tree<K, V, CAPACITY>(queue_length), block_size_(block_size) {
        }

        void create_and_init_blk_accessor() {
            this->blk_accessor_ = new nvme_blk_io_thread_accessor<K, V, CAPACITY>(block_size_);
...
#endif //NVM_IO_THREAD_B_PLUS_TREE_H
